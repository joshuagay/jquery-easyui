...
(function($){
	...
	function setSize(target) {
		...
		if ($.boxModel == true){
			hh = Math.max(
					$('.datagrid-view1 .datagrid-header',grid).height(),
					$('.datagrid-view2 .datagrid-header',grid).height()
			);
		} else {
			hh = Math.max(
					$('.datagrid-view1 .datagrid-header',grid).outerHeight(),
					$('.datagrid-view2 .datagrid-header',grid).outerHeight()
			);
		}
		...
		if (opts.height == 'auto') {
			...
			$('.datagrid-body', grid).height(
					...
					- $('.datagrid-header', grid).outerHeight()
					- $('.datagrid-title', grid).outerHeight()
					- $('.datagrid-toolbar', grid).outerHeight()
					- $('.datagrid-pager', grid).outerHeight()
			);
		}
	...
	function wrapGrid(target, rownumbers) {
		...
		grid.bind('resize', function(e,width,height){
			if (width && height){
